...
for ONEREGISTRYIN in ${AUTH_REGISTRIES}; do
    ...
    AUTH_USER_PASS=$(echo -n ${ONEREGISTRY} | cut -d "@" -f 2 | xargs)
    AUTH_USER=$(echo -n ${AUTH_USER_PASS} | cut -d ":" -f 1 | xargs)
    AUTH_PASS=$(echo -n ${AUTH_USER_PASS} | cut -d ":" -f 2 | xargs)
    AUTH_BASE64=$(echo -n ${AUTH_USER_PASS} | base64 | xargs)
